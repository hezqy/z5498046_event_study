...
def get_tics(pth):
    ...
    formatted_dic = {}
    with open(pth, 'r') as file0:
        for line in file0:
            exchange, ticker = line.strip().split('=')
            ticker = ticker.strip('"').lower()
            exchange = exchange.strip('"').lower()
            formatted_dic[ticker] = exchange
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        tic_exchange = tic_exchange_dic[tic]
        data_list = []
        dataline = read_dat(tic)
        for line in dataline:
            line_dic = line_to_dict(line)
            if col_lst:
                data_line_dic = {key: line_dic[key] for key in col_lst}
                data_list.append(data_line_dic)
            else:
                data_list.append(line_dic)
...
# ----------------------------------------------------------------------------
    """<COMPLETE THIS PART>
    (1)
    It helps configure the locations of files and folders in a portable and
    system-independent way. By using os module methods, it ensures that the paths
    are constructed correctly regardless of the operating system the code is
    executed, which means the code can run on any operating system without any
    modifications. Also, configuring as in step 1 allows users to move the project
    directory without having to update the paths in the code, making the code more
    flexible. Additionally, it brings ease of maintenance.

    In short, it ensures the project more portable, robust, flexible and easier
    to maintain.

    (2)
    Given the known observation that stock returns decrease in the short run
    when the articles contain relatively more negative words and with no reversal
    in the long run, we can analyse the rationality of these two hypotheses.

    1.Based on investors’ evaluations of those firms:

    1.1 Rationale:
    If the articles are based on investors’ evaluations of those firms, the
    negative terms may reflect investors' pessimistic sentiment or concern about
    the firms. In this case, investors may sell the stocks, resulting in decreased
    stock returns in the short run. And stock returns may not reverse in the long
    term, since investors' sentiment and evaluations usually persist.

    1.2 Consistency with Observation:
    This hypothesis is consistent with the known observation that stock returns
    decrease in the short run with no reversal in the long run.

    2. Based on valuable information beyond firm fundamentals:

    2.1 Rationale:
    If the articles are based on valuable information beyond firm fundamentals,
    then the negative terms in the articles may indicate or reveal some kind of risk
    or problem facing the firms. Such revelation may lead to the reassessment towards
    firms by investors, also resulting in decreased stock returns in the short run.
    As for the long run, if the market is efficient, this valuable information should
    be fully absorbed by the market and reflected in stock prices over the long term,
    leading to a reversal in stock returns.

    2.2 Consistency with Observation:
    This hypothesis is less consistent with the known observation of no reversal
    in stock returns in the long run.

    Conclusion:
    Based on the analysis, "the articles journalists wrote are based on investors’
    evaluations of those firms" is more likely to be true.

    (3)
    Given the hypothesis that the articles journalists wrote are based on investors’
    evaluations of those firms, the evaluation about the short-run predictability for
    the trading volume is as follows:

    Investors' sentiment can drive trading decisions and thereby affect trading
    volume. Since the articles are reflecting investors' evaluations, the sentiment
    expressed in the articles can potentially indicate the probable change of trading
    volume in the short run.

    When the articles contain relatively more negative words, it suggests a negative
    sentiment among investors towards the firms. Due to negative sentiment and concern
    about the firms' performance, investors may choose to short their holdings. As a
    result, trading volume can be expected to increase in the short run, as more
    investors participate in the market to sell stocks or to do other operations to the
    sentiment.

    However, trading volume is also impacted by other factors, such as news events. The
    sentiment is one of those factors. Therefore, it is crucial to realize the
    importance of market context and the limit of the predictability.

    In summary, based on the chosen hypothesis, trading volume is expected to increase
    in the short run, but it is limited by other factors that also influence trading volume.
    """
